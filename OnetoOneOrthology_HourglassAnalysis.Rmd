...
#provides gene affiliation to ids
kmeans_ALL1to1_genes<- function (mydata,k,iter, z) {
  kmeans<-list()
  kmeans$IDs <- data.frame()#accumulator for results
  kmeans$centers <- data.frame()
  ...
  #Combine cluster number and cost together, write to df
  kmeans$IDs<- rbind(kmeans$IDs, rbind(clusterID$cluster)) 
  kmeans$centers<- rbind.data.frame(kmeans$centers, rbind.data.frame(clusterID$centers))
  }
  #kmeans_df<-rbind.data.frame(kmeans_df, GeneClust=colnames(kmeans_df))
  row.names(kmeans$IDs)<-c(2:k)
  kmeans$IDs<-cbind.data.frame(sp=rep(z, k-1), k=2:k, kmeans$IDs)
  k.centerNum<-c()
  k.numID<-c()
  for (i in 2:k) {
  k.center<-paste(i,".", 1:i, sep="")
  k.centerNum<-append(k.centerNum, k.center)
  k.num<-paste(rep(i, i))
  k.numID<-append(k.numID, k.num)
  }
  k_num=data.frame(k.numID)
  kmeans$centers<-cbind.data.frame(k_num, kmeans$centers)
  row.names(kmeans$centers)<-k.centerNum
  return(kmeans) 
  }
...
Look at kmeans of contrast data each 1 to 1 ortholog phylo-contrast by the evolutionary type they fall in. Rows indicate the number of clusters
```{r generate_ConstrastEvoType}
Amniote_1to1_EvoType<-kmeans_ALL1to1_genes(t(Amniote_1to1_Rank_contrast_clust5), 20, 15, "Amniote")
Frog_1to1_EvoType<-kmeans_ALL1to1_genes(t(Frog_1to1_Rank_contrast_clust5), 20, 15, "Frog")
Anamniote_1to1_EvoType<-kmeans_ALL1to1_genes(t(Anamniote_1to1_Rank_contrast_clust5), 20, 15, "Anamniote")
Vertebrate_1to1_EvoType<-kmeans_ALL1to1_genes(t(Vertebrate_1to1_Rank_contrast_clust5), 20, 15, "Vertebrate")
```
